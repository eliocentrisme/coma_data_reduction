# phaseshift(vis='coma_lbandD0330.ms', outputvis='coma_lbandD0330_shifted.ms', phasecenter='J2000 12.54.11 +27.37.30')
...
msfile = 'coma_lbandD0415.ms'
last_image = 'coma_lbandD0415_last'
...
for idx, solint in enumerate(solution_intervals):
    image_name = f'selfcal_images/selfcal_cycle_{idx}'
    gain_table = f'selfcal/gains_cycle_{idx}.cal'
...
# wsclean_cmd = f"singularity exec {singularity_image} wsclean -size 5000 5000 -scale 2asec -data-column CORRECTED_DATA -update-model-required -reorder -weight briggs 0.0 -clean-border 1 -parallel-reordering 4 -gain 0.1 -mgain 0.7 -nmiter 30 -padding 1.4 -join-channels -channels-out 16 -local-rms -local-rms-strength 0.5 -auto-mask 3.0 -auto-threshold 0.5 -multiscale -multiscale-scale-bias 0.8 -multiscale-max-scales 7 -fit-spectral-pol 3 -pol i -gridder wgridder -parallel-deconvolution 2000 -fit-beam -name selfcal_images/selfcal_cycle_bp -niter 300000 {msfile}"
