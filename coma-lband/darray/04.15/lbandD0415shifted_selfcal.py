# phaseshift(vis='coma_lbandD0415.ms', outputvis='coma_lbandD0415_shifted.ms', phasecenter='J2000 12:54:11.712400 +27.37.30.465000')
...
msfile = 'coma_lbandD0415_shifted.ms'
last_image = 'shifted_last'
...
# last_ptable = f'selfcal/shifted_gains_cycle_9.cal'
gain_solutions = []
...
gaincal(vis=msfile, caltable=f'selfcal/{last_image}.cal', solint='100s', refant='ea09', gaintype='G', calmode='p')

applycal(vis=msfile, gaintable=f'selfcal/{last_image}.cal', calwt=False)

for idx, solint in enumerate(solution_intervals):
    image_name = f'selfcal_images/shifted_selfcal_cycle_{idx+4}'
    gain_table = f'selfcal/shifted_gains_cycle_{idx+4}.cal'
    
    wsclean_cmd = f"singularity exec {singularity_image} wsclean -size 5000 5000 -scale 2asec -data-column CORRECTED_DATA -update-model-required -reorder -weight briggs 0.0 -clean-border 1 -parallel-reordering 4 -gain 0.1 -mgain 0.7 -nmiter 30 -padding 1.4 -join-channels -channels-out 16 -fits-mask selfcal_images/shifted_mask.mask.fits -multiscale -multiscale-scale-bias 0.8 -multiscale-max-scales 7 -fit-spectral-pol 3 -pol i -gridder wgridder -parallel-deconvolution 2000 -fit-beam -name {image_name} -niter 300000 {msfile}"
...
wsclean_cmd = f"singularity exec {singularity_image} wsclean -size 5000 5000 -scale 2asec -data-column CORRECTED_DATA -update-model-required -reorder -weight briggs 0.0 -clean-border 1 -parallel-reordering 4 -gain 0.1 -mgain 0.7 -nmiter 30 -padding 1.4 -join-channels -channels-out 16 -fits-mask selfcal_images/shifted_mask.mask.fits -multiscale -multiscale-scale-bias 0.8 -multiscale-max-scales 7 -fit-spectral-pol 3 -pol i -gridder wgridder -parallel-deconvolution 2000 -fit-beam -name selfcal_images/shifted_selfcal_cycle_last -niter 300000 {msfile}"
